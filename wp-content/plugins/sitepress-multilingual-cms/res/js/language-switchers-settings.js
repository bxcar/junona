...
WPML_core.languageSwitcher = (function( $, wpml_ls ) {
	...
	var repositionDialog = function () {
		...
		//dialogBox.dialog("option", "maxWidth", winW);
		dialogBox.dialog("option", "maxHeight", winH);
		//$('.wpml-ls-subform-options').css('max-height', parseInt(dialogBox.css('max-height')) - $('.js-wpml-ls-preview-wrapper').outerHeight());
		...
		if(preview.length > 0) {
			...
			wp.ajax.send({
				...
				success:  function (data) {
					...
					buildCSSLinks(data.css, preview);
					buildJSScripts(data.js, preview);
	...
	var updateSettings = _.debounce(function (wrapper, successCallback) {
		...
		wp.ajax.send({
			...
			success:  function (data) {
				showUpdatedContent(messageWrapper, data, 1000, 'updated');
			...
			error: function (data) {
				showUpdatedContent(messageWrapper, data, 2000, 'error');
			}
		});
	}, 500);
	...
	var buildCSSLinks = function (css, preview) {
		var i,
			linkTag,
			previewWrapper = preview.closest('.js-wpml-ls-preview-wrapper');

		previewWrapper.find('link').remove();
		...
		if (typeof css !== typeof undefined && null !== css) {
			for (i = 0; i < css.length; i++) {
				...
				linkTag.prependTo(previewWrapper);
			}
		}
	};

	var buildJSScripts = function (js, preview) {
		var i,
			jsTag,
			previewWrapper = preview.closest('.js-wpml-ls-preview-wrapper');

		previewWrapper.find('script').remove();
		...
		if (typeof js !== typeof undefined && null !== js) {
			for (i = 0; i < js.length; i++) {
				...
				jsTag.attr('src', js[i]);
				jsTag.prependTo(previewWrapper);
			}
		}
	};
