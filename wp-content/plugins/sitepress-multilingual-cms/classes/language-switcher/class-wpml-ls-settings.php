...
class WPML_LS_Settings extends WPML_SP_User {

	/* @var string $settings_slug */
	private $settings_slug = 'wpml_language_switcher';

	/* @var array $settings */
	private $settings;

	/* @var array $settings_preset */
	private $settings_preset;
	 ...
	 * @param array                            $core_templates
	 */
	public function __construct( $templates, $sitepress, $slot_factory, $core_templates ) {
	...
	public function init_hooks() {
		...
		add_action( 'init',                           array( $this, 'init_action' ), 2000 );
		add_filter( 'wpml_config_array',              array( $this, 'wpml_config_array_filter' ) );
	}

	public function init_action() {
		$this->restore_preset_settings();
	}

	private function restore_preset_settings() {
		$is_initialized = $this->sitepress->get_setting( 'language_selector_initialized' );

		if ( empty( $is_initialized )
			|| ( isset( $_GET[ 'restore_ls_settings' ] ) && $_GET[ 'restore_ls_settings' ] == 1 )
		) {
			delete_option( $this->settings_slug );
			$this->settings = null;

			if ( $this->settings_preset ) {
				$this->save_settings( $this->settings_preset );
			} else {
	 ...
	 * @return array
	 */
	private function read_preset_settings_recursive( $arr ) {
		$ret = array();

		foreach ( $arr as $v ) {
			if ( isset( $v['key'] ) && is_array( $v['key'] ) ) {
				$partial = ! is_numeric( key( $v['key'] ) ) ? array( $v['key'] ) : $v['key'];
				$ret[ $v['attr']['name'] ] = $this->read_preset_settings_recursive( $partial );
			} else {
	 ...
	 * @return mixed
	 */
	public function wpml_config_array_filter( $config_all ) {
		$this->settings_preset = isset( $config_all['wpml-config']['language-switcher-settings']['key'] )
			? $config_all['wpml-config']['language-switcher-settings']['key'] : null;

		$this->settings_preset = $this->read_preset_settings_recursive( $this->settings_preset );

		if ( strpos( key( $this->settings_preset ), 'icl_') !== 0 ) {
			unset( $config_all['wpml-config']['language-switcher-settings'] );
		}
	...
	private function maybe_init_settings() {
		if ( null === $this->settings ) {
			$this->settings = get_option( $this->settings_slug );

			if ( ! $this->settings || ! isset( $this->settings['migrated'] ) ) {
				$migration = new WPML_LS_Migration( $this, $this->sitepress, $this->slot_factory );
				$converted_settings = $migration->get_converted_settings();
				$this->save_settings( $converted_settings );
			}
	...
	public function get_menu_settings_from_id( $menu_id ) {
		...
		$nav_menu        = $menu_element->get_source_language_code() ?
			$menu_element->get_source_element()->get_wp_object() : $menu_element->get_wp_object();
	...
	public function save_settings( $new_settings ) {
		$this->maybe_init_settings();
		$new_settings             = $this->preserve_hidden_settings( $new_settings );
	 ...
	 * @return mixed
	 */
	private function preserve_hidden_settings( $new_settings ) {
		$preserved = array( 'migrated' );

		foreach ( $preserved as $key ) {
			$new_settings[ $key ] = isset( $new_settings[ $key ] )
				? $new_settings[ $key ] : ( isset( $this->settings[ $key ] ) ? $this->settings[ $key ] : null );
		}
	...
	private function get_slot_from_widget_instance( $widget_instance ) {
		...
		if ( ! is_a( $slot, 'WPML_LS_Sidebar_Slot' ) ) {
			$slot = $this->get_default_slot_arguments( 'sidebars' );
