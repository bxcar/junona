...
class WPML_LS_Templates {
	...
	private function init_available_templates() {
		...
		// todo: change the condition to match "WPML -> languages" page during integration
		if ( ! is_admin() || defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			$this->templates = get_site_transient( self::TRANSIENT_NAME );
		}
		...
		if ( $this->templates === false ) {
			...
			foreach ( $templates_paths as $template_path ) {
				$template_path = $this->fix_dir_separator( $template_path );
				if ( file_exists( $template_path . $this->ds . WPML_LS_Template::FILENAME ) ) {
					...
					$tpl['base_uri'] = trailingslashit( $this->get_uri_from_path( $template_path ) );
					...
					$tpl['flags_base_uri'] = isset( $config['flags_dir'] ) // todo: check with ../
						? $this->get_uri_from_path( $template_path . $this->ds . $config['flags_dir'] ) : null;
					$tpl['flags_base_uri'] = ! isset( $tpl['flags_base_uri'] ) && file_exists( $template_path . $this->ds . 'flags' )
						? $this->get_uri_from_path( $template_path . $this->ds . 'flags' ) : $tpl['flags_base_uri'];
	...
	private function get_files( $ext, $template_path, $config ) {
		...
		if( isset( $config[ $ext ] ) ) {
			...
			foreach ( $config[ $ext ] as $file ) {
				...
				$resources[] = $this->get_uri_from_path( $file );
			}
		} else {
	 ...
	 * @return string
	 */
	private function get_uri_from_path( $path ) {
		$relative_path = substr( $path, strlen( ABSPATH ) );
		return site_url() . '/' . str_replace( array('/', '\\' ), '/', $relative_path );
	}
	...
	private function get_unique_name( $name, $path ) {
		...
		if ( strpos( $path, $this->fix_dir_separator( get_template_directory() ) ) === 0 ) {
			$theme = wp_get_theme();
			$name = $theme . ' - ' . $name;
		} else if (
			strpos( $path, $this->fix_dir_separator( WP_PLUGIN_DIR ) ) === 0
			&& ! $this->is_core_template( $path )
		) {
			$plugin_dir = $this->fix_dir_separator( WP_PLUGIN_DIR );
